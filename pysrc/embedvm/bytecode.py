...
class RefDataCommand(object):
    """Mark the command for the assembler to fill in a dataptr attribute once
    it can resolve symbols (like VariableAddressCommand)"""

class PushRef(Push16, RefDataCommand):
    """Push a function or data pointer. As this yields a push instruction just
    as a push constant would do, this can not be generated by disassembling.
    Passing around function pointers is inherently non-relocatable, and will
    thus potentially break in a disassemble / unfix / fix / assemble roundtrip."""

    def __init__(self, value):
        self.data = value

    value = property(lambda self: self.dataptr)
...
class VariableAddressCommand(VariableLengthCommand):
    """Among other things, this class marks the command for the assembler to
    fill the reladdr attribute once the labels can be resolved"""
